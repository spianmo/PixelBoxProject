...
2. 红绿灯:真件在原生全屏下被 AppKit 收进顶部悬停工具条(Electron 公开 API 无法钉住,
   见 electron#21604),TitleBar 在原位**自绘假红绿灯**——红=关闭、黄=禁用(macOS 全屏
   下最小化不可用,同语义)、绿=退出全屏(原生 setFullScreen 同一入口);组悬停显示
   符号;鼠标移到屏幕顶部呼出系统工具条时真件会短暂与假件同屏(系统行为,无害);
   退出全屏恢复 80px 预留区与真件;系统菜单栏是否在全屏显示跟随系统设置;
