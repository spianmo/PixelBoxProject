...
2. 红绿灯:真件在原生全屏下被 AppKit 收进顶部悬停工具条(Electron 公开 API 无法钉住,
   见 electron#21604),TitleBar 在真件原位**自绘假红绿灯**——红=关闭、黄=禁用(macOS
   全屏下最小化不可用,同语义;悬停仍显 − 符号)、绿=退出全屏(原生 setFullScreen
   同一入口,悬停显全屏态「相向双三角」符号);**与真件一比一**(测量驱动):绝对定位
   到真件实测圆位(12pt 圆、圆心距 20pt,圆心 (18.75,17.75) 起——trafficLightPosition
   {x:12,y:10} 名义位在 Tahoe 实测内缩 (0.75,1.75)pt),描边/填充/悬停符号几何全部按
   真件截屏基准 crop 实测调整(见 `scripts/traffic-lights-diff-check.mjs` 头注);
   80px 预留区窗口态/全屏态恒定,内容不因假件挤动;组悬停三枚同时出符号;鼠标移到
   屏幕顶部呼出系统工具条时真件会短暂与假件同屏(系统行为,无害);退出全屏恢复真件;
   系统菜单栏是否在全屏显示跟随系统设置;
...
7. 视觉级验证(真实 mac GUI,`pnpm run check:fullscreen`):dev 启动 → 程序化进原生
   ...
   不可用则如实降级为 AppKit 状态断言并打印授权指引)→ PIL 像素断言(均为硬断言):
   **假红绿灯三簇常驻可见**(名义色同真件,x 序 红<黄<绿)、标题栏横带众数 ≈ 主题色
   (dark #2B2D30 / light #F7F8FA,无灰条,双候选行兼容菜单栏叠占)、退出 bounds 恢复,
   打印 `[fs-visual] PASS/FAIL`。注意:macOS **锁屏状态下系统会拒绝全屏 Space 过渡**,
   本检查需在解锁的 GUI 会话运行;
8. 红绿灯一比一验收(真实 mac GUI,`pnpm run check:lights`):dev 启动(窗口态已知
   矩形)→ 截真件基准(常态 + python3 Quartz 合成鼠标悬停红键触发组悬停)→ 程序化进
   原生全屏 → 同法截假件 → 三枚各裁 24×24pt(@2x)crop(ICC→sRGB)逐通道平均绝对差:
   常态各 ≤10/255、悬停各 ≤18/255(符号抗锯齿 + 全屏绿键符号变体,放宽),crop 落盘
   `/tmp/pb-tl-crops/` 供人工复核,打印每枚 diff 与 `[tl-diff] PASS/FAIL`;合成悬停
   不可用/未生效时如实降级(仅验常态并标注悬停未验)。依赖:`pillow` +
   `pyobjc-framework-Quartz`(悬停);同样需解锁 GUI 会话与屏幕录制授权。
